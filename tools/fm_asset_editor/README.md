...
|---|---|
| 左ドラッグ | 選択中の色で描く |
| 右クリック | **色拾い** (指した画素の色を選択色にする。道具が何でも効く)。Settings メニューで「消す (透明、シートなら 0)」に変えられ、選択は次回起動にも残る |
| 1 / 2 / 3 | 鉛筆 / 塗りつぶし / 色拾い |
| + / - | 拡大・縮小 |
| g | 格子の表示切替 |
...
右クリックの役割 (Settings メニュー) も同じファイルに入る。読めない値は
既定の色拾いに戻る。
...
- **パートの長さの食い違いの指摘**: 曲の長さは一番長いパートで決まるので、
  短いパートは繰り返しまで黙って待ち、長くなった原因の音はその間ずっと違う
  小節に鳴っている。`part 1 is 24 clocks (1 beat) shorter than the longest
  part` のように、拍・小節に直して出す
